...
void txButtonPressMessage(){
  ...
  // Write our ID to byte zero of the buffer "b"
  // ??

  // Write the word "press" to the buffer "b"
  b[1] = 'p';
  // ??
  // ??
  // ??
  ...
  {
    // Display a message indicating that a message
    // was transmitted.
  }
  else
  {
...
void loop()
{
  ...
  if (buttonPressed && !buttonPressedNow)
  {
    ...
    // Send one message if the timePressed is long and a different one if
    // timePressed is short
